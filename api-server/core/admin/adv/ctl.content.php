...
class ctl_content extends adminPage
{
	...
	function ctl_content ()
	{
		...
		$this->refreshUrl = '?con=admin&ctl=adv/content&pid='.$this->pcat['id'].'&level='.$this->level.'&';
	}
	...
	public function add_action () #act_name = 添加#
	{
		...
		if ( is_post() )
		{
			...
			if ( empty( $data['template'] ) ) $data['template'] = $parent['template'] ? $parent['template'] : 'content';
