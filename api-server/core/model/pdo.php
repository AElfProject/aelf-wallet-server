...
class mdl_pdo {
	...
	public function mdl_pdo ( $db, $tableName, $lang = false ) {
		$this->db = $db;
	...
	public function getByWhere( $where, $colunms = array(), $order = '' ) {
		if ( ! isset( $where['lang'] ) && $this->lang ) {
			$where['lang'] = $this->getLang();
		}

		return $this->db->selectOne( $colunms, $this->tableName, $where, $order );
	}

	public function getList( $column = null, $where = null, $order = null, $cnt = null ) {
            if($cnt and strpos($cnt,',')){
            $limit = $cnt;
        }else{
            $cnt = (int)$cnt;
            $limit = $cnt > 0 ? " 0, $cnt" : '';
		...
		return $this->db->select( $column, $this->tableName, $where, $order, $limit );
	}
