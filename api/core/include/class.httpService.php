...
class httpService
{
	...
	function httpService ($url, $data = array(), $abort = false)
	{
		$this->url	= $url;
		$this->data	= $data;
	...
	private function getByFOpen ($url)
	{
		$fp		= fopen($url, 'rb');
		$str	= '';
		while(true)
	...
	public function result ()
	{
		...
		{
			if (isset($this->charset) && strtolower($this->charset) == 'gb2312') return g2u(self::getByFOpen($this->url));
			else return self::getByFOpen($this->url);
		}
	}
